...
def conv_block(in_channels, out_channels, pool=False, start=False, track_running_stats=False):
    layers = [SlimmableConv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False, start=start),
              SlimmableBatchNorm2d(out_channels, track_running_stats=track_running_stats),
...
class SlimmableResNet9_tinyimagenet(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super().__init__()

        self.conv1 = conv_block(in_channels, 64, start=True, track_running_stats=True)
        self.conv2 = conv_block(64, 128, pool=True, track_running_stats=True)
        self.res1 = nn.Sequential(conv_block(128, 128, track_running_stats=True),
                                  conv_block(128, 128, track_running_stats=True))
        ...
        self.res2 = nn.Sequential(conv_block(512, 512, track_running_stats=True),
                                  conv_block(512, 512, track_running_stats=True))
        if num_classes == 200:
            self.classifier = nn.Sequential(nn.MaxPool2d(8),
                                            nn.Flatten(),
                                            SlimmableLinear(512, num_classes, bias=False, end=True))
